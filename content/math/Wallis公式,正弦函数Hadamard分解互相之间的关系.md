...
1.  Hadamard分解定理:$$\sin(\pi z)=e^{g(z)}z^m\prod_{n=1}^{\infty}E_k\left(z/z_n\right)$$因为这个函数的零点就是全体的整数。$k = \lfloor \rho\rfloor$，$\rho$是函数的增长阶其中$E_k(w)$是Canonical factor，其形式为，  $$E_0(w)=1-w \quad \text{and}\quad
