...
于是我们完全可以把图1当中的矩形的路径$\gamma_R$理解为两个类似于图2当中的半个矩形的积分路径的差，假设图2的路径为$\gamma_{R_2}$，那么$$\gamma_R=\gamma_{R_1}-\gamma_{R_2}$$现在可以尝试把这个式子带入到傅里叶逆变换的一半的公式里面，之所以是一半，是因为计算到最后的时候会出现
$$
\int_{0}^{\infty}e^{
-(2\pi b+i(2\pi u-2\pi x))\xi}\,d\xi
$$
如果我们令复数$s=2\pi b+i(2\pi u-2\pi x)$那么这就是一个对常函数$1$做lapalce变换，从而结果就是$1/s$。 同样的道理，如果傅里叶逆变换公式的另一半去计算就会得到
$$
