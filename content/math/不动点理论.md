...
---
## 分析观点下的不动点
压缩映射是不动点和Lipschitz条件的基础
在完备的度量空间上若$f([a,b])\subset [a,b]$并存在0<k<1,则成立${|f(x)-f(y)|}\leq{k|x-y|}$
即${d(Tx,Ty)}\leq {d(x,y)}$
Banach不动点定理

> [!note] Picard-Banach不动点定理
...
具体来说$$d\left(x_{n+1},x_n\right)\leq qd\left(x_n,x_{n-1}\right)\leq\cdots\leq q^nd\left(x_1,x_0\right)$$如果考虑Cauchy判别去判断收敛性，那么对于任意的正整数k考虑是否有$d(x_{n+k},x_n)\to0$ 与$k$无关：$$\begin{aligned}d\left(x_{n+k},x_n\right)&\le q d\left(x_n,x_{n+1}\right)+\cdots+d\left(x_{n+k-1},x_{n+k}\right)\leq\\&\leq\left(q^n+q^{n+1}+\cdots+q^{n+k-1}\right)d\left(x_1,x_0\right)\leq\frac{q^n}{1-q}d\left(x_1,x_0\right)\to0\end{aligned}$$这里完备性主要用在,要在$X$当中存在不动点$x\in X$,就需要$x_n\in X,\forall n \in\mathbb{N}$其极限要存在于$X$当中。

>[!问题1]
>数列{$b_n$}由$b_1=1$和${b_{n+1}}=1+{1\over{b_n}}$生成，求收敛极限

尝试构造${|f(x)-f(y)|}\leq{k|x-y|}$，有${|f(x)-f(y)|}={|{1\over x}-{1\over y}|}$
注意到${f([1.5,2]}\subset{[1.5,2]})$
则${|f(x)-f(y)|}={|{1\over x}-{1\over y}|}\leq{{1\over 1.5^2}|x-y|}$
我们称$b_n$从第二项进入$[1.5,2 ]$,因此由压缩压缩保证齐收敛性
再由不动点定理得极限为${(1+\sqrt 5)}\over{2}$

>[!问题2]
>数列{$b_n$}由$b_0={{\sqrt2}\over 2 }$和${b_{n+1}}={1\over {2b_n}}+{{1\over2}b_n}$生成，求收敛极限


>[!核心想法]
>为确定范围，假设存在一个上下界a,b，则$a={1\over 2}{(a+{1\over b})}\leq {b_n}\leq {1\over 2}{(b+{1\over a})}=b$
>得ab=1，又$b_0={{\sqrt2}\over 2 }$所以取$\sqrt2$

当n=0时$b_0={{\sqrt2}\over 2 }$成立
假设${{\sqrt2}\over 2 }\leq b_n\leq {\sqrt2}$
则${x_{n+1}}={{1\over 2}{{x_n}+{1\over x_n}}}\leq{{\sqrt2}\over{2}}+{1\over{2}}=\sqrt2$
又${x_{n+1}}={{1\over 2}{{x_n}+{1\over x_n}}}\geq{{\sqrt2}\over{4}}+{1\over{{2}\sqrt2}}={{\sqrt2}\over2}$
则满足${f([{{\sqrt2}\over 2 },{\sqrt2}]})\subset[{{\sqrt2}\over 2 },{\sqrt2}]$
$f'(x)=|{{1\over 2}{1-{1\over {x^2}}}}|\subset{[0,{1\over2}]}$
${|f(x)-f(y)|}={f'(\xi)|x-y|}\leq|x-y|$
则称其收敛由压缩映射保证，又由不动点得极限为1
...
那么一串珠子一共有哪些对称和反射呢？自然是这个几何图形的对称群决定的。问题中的8个珠子组成的项链，可以等价为一个正8边形，因此对称群自然是八阶Dihedral群$D_8$。
...
3. 那么群$G$关于映射的核$G_x$的陪集做成的集合$G/G_x$的数量和映射的像的数量一样，也就是$$|G/G_x|= |O_x|$$这个关系主要是来自于第一同构定理。这并非群的第一同态，集合$X$并非群，我们单纯从集合的角度理解$$G \twoheadrightarrow G/\text{ker}f_x
