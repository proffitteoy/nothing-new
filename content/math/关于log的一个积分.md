...
1.  从一致收敛的角度考虑(R积分):不难求证:$|x^n\log(x)|\leq \frac{1}{en},n \geq1$ 因为在(0,1)内存在最小值0,1部分的极限都是0.所以$\left|\frac{x^n\log(x)}{n}\right| \leq \frac{e^{-1}}{n^2}$，因此部分和 $S_N(x)\rightrightarrows S(x)$，于是在紧集$0,1]$上，
$$
\int_{0}^{1}S_N(x) dx \to
    \int_{0}^{1}S(x)dx
$$
...
3.  从Fubini -Tonelli定理的角度考虑(L积分):这么做的技巧在于把求和看成积分。$\Omega=\{1,2,...\}$,然后定义counting measure $\mu$，然后考虑在sigma-finite测度空间$(\Omega,\mathcal{P}(\Omega),\mu)$的积分，
$$
\int_{\Omega}
...
$$
用Tonelli定理交换因为被积分对象非负，因为原来的函数$f(x,n)=\frac{x^n\log(x)}{n}$是可测函数。
