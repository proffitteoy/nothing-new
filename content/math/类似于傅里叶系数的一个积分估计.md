...
不能把$\int_0^1 f(x)g(nx)\,dx$拆成$\left(\int_0^1 f(x)\,dx\right)\left(\int_0^1 g(nx)\,dx\right).$
一般只能估计：

$$
\left|\int_0^1 f(x)g(nx)\,dx\right|
\leq
\int_0^1 |f(x)||g(nx)|\,dx
\leq
\|f\|_\infty \int_0^1 |g(nx)|\,dx.
$$

也就是 $\{a_n\}$ 有界，不能说明 $a_n\to0$，更不能说明 $\sum a_n^2$ 收敛。
...
\int_0^1 g(nx)\,dx
=
\frac1n\int_0^n g(y)\,dy
=
\frac1n\sum_{k=0}^{n-1}\int_k^{k+1}g(y)\,dy
=
0,
$$

但这不代表$\int_0^1 f(x)g(nx)\,dx=0.$因为 $g(nx)$ 自己的正负部分可以抵消，但乘上 $f(x)$ 后，不同位置被赋予了不同权重，抵消一般不再完全成立。
...
而$a_n$的形式又很难不让我们想到广义的Riemann - Lebesgue lemma当中的结果，此结果告诉我们$$a_n\to \left(\int_{0}^{1} f(x) dx
