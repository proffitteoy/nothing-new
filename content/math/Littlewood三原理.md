...
>[!note] Lusin定理（1912）
> 假设$f$是一个Lebesgue可测函数，并且在一个有限测度（Lebesgue测度）集合$E\subset \mathbb{R}^d$上取值有限。那么对任意的$\varepsilon>0$存在一个闭集$F_{\varepsilon}$使得$$F_{\varepsilon}\subset E,\quad m(E-F_{\varepsilon})<\varepsilon$$并且使得$f|_{F_{\varepsilon}}$是连续的。
