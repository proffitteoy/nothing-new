...
export default function MusicClient() {
          ...
          <section className="w-full max-w-md rounded-3xl border border-white/40 bg-white/45 p-8 shadow-xl backdrop-blur-xl transition-colors duration-700 dark:border-white/10 dark:bg-slate-800/50">
            <div className="mx-auto mb-8 flex h-24 w-24 items-center justify-center rounded-3xl border border-white/50 bg-white/50 shadow-inner dark:border-white/10 dark:bg-slate-900/50">
              <Disc3
                ...
                className={
                  isLoading
                    ? "animate-spin text-indigo-500 dark:text-indigo-300"
  ...
  return (
    <div className="relative min-h-[100svh] overflow-hidden text-slate-900 dark:text-white">
      ...
      >
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-2 sm:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)_auto]">
          ...
          <div
            aria-live="polite"
            aria-atomic="true"
            title={currentLyric || "暂无歌词"}
            className="relative col-span-2 row-start-2 h-6 min-w-0 overflow-hidden text-center sm:col-span-1 sm:col-start-2 sm:row-start-1"
          >
            {reduceMotion ? (
              <p className="truncate text-sm font-bold leading-6 text-slate-600 dark:text-slate-300">
                {currentLyric || "暂无歌词"}
              </p>
            ) : (
              <AnimatePresence initial={false}>
                <motion.p
                  key={currentSong.id + "-" + (currentLyric || "暂无歌词")}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.28, ease: "easeOut" }}
                  className="absolute inset-0 truncate text-sm font-bold leading-6 text-slate-600 dark:text-slate-300"
                >
                  {currentLyric || "暂无歌词"}
                </motion.p>
              </AnimatePresence>
          ...
          <div className="col-start-2 row-start-1 flex shrink-0 items-center gap-1 sm:col-start-3 sm:gap-2">
            <button
              type="button"
            ...
            <button
              ...
              aria-label="上一首"
              onClick={prevSong}
              className="flex h-10 w-10 items-center justify-center rounded-full text-slate-700 transition hover:bg-white/70 hover:text-indigo-600 dark:text-slate-200 dark:hover:bg-white/10 dark:hover:text-indigo-300"
            >
            ...
            <button
              ...
              onClick={togglePlay}
              className="flex h-12 w-12 items-center justify-center rounded-full bg-indigo-500 text-white shadow-lg shadow-indigo-500/30 transition hover:scale-105 hover:bg-indigo-600"
            >
              {isPlaying ? (
                <Pause size={23} fill="currentColor" aria-hidden="true" />
              ) : (
                <Play size={23} fill="currentColor" className="ml-0.5" aria-hidden="true" />
              )}
            </button>
            <button
              type="button"
              ...
              <AnimatePresence>
                {showVolumeSlider && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 8 }}
                    className="absolute bottom-12 right-0 rounded-full border border-white/50 bg-white/90 px-4 py-3 shadow-lg backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/90"
                  >
                    <input
                      type="range"
                      min="0"
                      ...
                      style={{
                        background:
                          "linear-gradient(to right, #ec4899 " +
                          (volume || 0) * 100 +
              ...
              <button
                ...
                className={
                  ...
                  (showVolumeSlider
                    ...
                    : "text-slate-600 hover:bg-white/70 hover:text-pink-600 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-pink-300")
                }
              >
                {isMuted || volume === 0 ? (
            ...
            <button
              ...
              aria-expanded={queueOpen}
              onClick={() => setQueueOpen(true)}
              className="flex h-10 w-10 items-center justify-center rounded-full text-slate-600 transition hover:bg-white/70 hover:text-indigo-600 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-indigo-300 sm:hidden"
            >
        ...
        <div className="mt-3 flex items-center gap-3">
          <span className="w-10 text-right text-[11px] font-black tabular-nums text-slate-500 dark:text-slate-400">
          ...
          />
          <span className="w-10 text-[11px] font-black tabular-nums text-slate-500 dark:text-slate-400">
            {formatTime(duration)}
          </span>
        </div>
      </section>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-200/60 p-4 dark:border-white/10">
                <div>
                  <h2 id="music-queue-title" className="text-xl font-black">
                    播放列表
                  </h2>
                  <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                    {playlist.length} 首歌曲 · {playModeLabel}
                  </p>
                </div>
                <button
                  type="button"
                  aria-label="关闭播放列表"
                  onClick={() => setQueueOpen(false)}
                  className="flex h-10 w-10 items-center justify-center rounded-full text-slate-600 transition hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white"
                >
                  ...
                  {playlist.map((song, index) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={
                          "grid grid-cols-[52px_minmax(0,1fr)] items-center gap-3 rounded-2xl border p-3 text-left transition " +
                          (isActive
                            ? "border-indigo-400/50 bg-indigo-500/10"
                            : "border-transparent hover:border-indigo-300/40 hover:bg-white/65 dark:hover:border-white/10 dark:hover:bg-white/5")
                        }
                      >
                          ...
                          {isActive && (
                            <span className="absolute inset-0 flex items-center justify-center bg-indigo-600/55">
                              <AudioLines
                                size={17}
                                className={
                                  isPlaying ? "animate-pulse text-white" : "text-indigo-100"
                                }
                                aria-hidden="true"
                              />
                            </span>
      ...
      <style jsx global>
        {
          ".music-scrollbar::-webkit-scrollbar{width:8px}.music-scrollbar::-webkit-scrollbar-track{background:rgba(148,163,184,.14);border-radius:999px}.music-scrollbar::-webkit-scrollbar-thumb{background:linear-gradient(180deg,rgba(99,102,241,.86),rgba(236,72,153,.58));border-radius:999px}"
        }
      </style>
    </div>
  )
