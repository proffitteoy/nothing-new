...
"问题0"当中的不等式$|f'(x)|\leq A |f(x)|$结合微积分基本定理，以及初值条件我们可以得到
$$
f(x)=\int_{0}^{x} f'(t)\,dt \implies |f(x)|\leq \int_{0}^{x}|f'(t)|\,dt \leq A\int_{0}^{x} |f(t)|\,dt\tag{1}
$$
于是我们可以定义这样一个保偏序关系的映射$$L:f\mapsto \int_{0}^{x} f(t)\,dt$$从而$(1)$可以等价于$$|f|\leq L(|f|)$$根据“命题2.1”当中的抽象Gronwall不等式，我们可以得到
$$
|f|\leq L(g)\tag{2}
$$
其中$g$是$L$在
