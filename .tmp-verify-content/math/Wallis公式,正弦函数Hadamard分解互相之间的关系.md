...
然后令$z =1/2$，那么会得到
$$
1=\frac{\pi}{2} e^a \prod_{m\geq
    1}\left(1-\frac{1}{2m}\right)\left(1+\frac{1}{2m}\right)
$$
由Wallis公式的结果，得到
$$
e^a = 1
...
$$
因为如果这样，令$G(z) = \frac{\sin(\pi z)}{\pi},P(z) = z\prod_{n\geq 1}\left(1-\frac{z^2}{n^2}\right)$,那么可以如果可以证明$G(z)/P(z)=1$那么问题便解决了，而证明这件事首先要证明这俩的商是常数，于是便考虑微分。这个想法很自然，因为他们的零点可以互相抵消，而且$z\to 0$的时候的确$G(z)/P(z) \to 1$,所以只要证明微分是0即可。
$$
