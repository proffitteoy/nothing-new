...
- **Field Mode 组件避让**：关于页资料卡、杂谈/番剧卡片与阅读面板登记实际 DOM 边界，前后景沿用包围圆避让，不截断可见障碍列表。友链标题、卡片和申请面板也保持干净，粒子留在页面空白与卡片间隙；全局导航和播放器仍保留保护。不要把整页或撑满高度的网格列标记为障碍物。
- **跨页面音乐体验**：音乐页提供完整播放界面，桌面端同时保留全局悬浮控制器。
- **项目与兴趣归档**：项目页采用可环绕的窗边研究小屋，保留静态预览和可读目录；项目内容由本地数据维护，番剧页通过服务端 Bangumi API 获取收藏数据。
...
- <code>app/projects/projects.ts</code>：项目档案与物件映射；模型重建与验收见 [窗边研究小屋](docs/project/projects-room.md)。
